#!/usr/bin/env python3
# Student name: Xueqing (Jessica) Zhai
...
class PartialParse(object):
    ...
    def get_n_leftmost_deps(self, sentence_idx, n=None):
        ...
        """
        # *** BEGIN YOUR CODE *** #
        # print(f"\n\nsentence idx: {sentence_idx}\n")
        # print(f"left most start, arcs: {self.arcs}\n")
        deps = []
        if n is None:
        ...
        # *** END YOUR CODE *** #
        # print(f"left most end, deps: {deps}\n")
        return deps
    ...
    def get_n_rightmost_deps(self, sentence_idx, n=None):
        ...
        # *** BEGIN YOUR CODE *** #
        # print(f"\n\nsentence idx: {sentence_idx}\n")
        # print(f"right most start, arcs: {self.arcs}\n")
        self.arcs.reverse()
        # print(f"right most start, reversed arcs: {self.arcs}\n")
        deps = []
        if n is None:
            for arc in self.arcs:
                if arc[0] == sentence_idx:
        ...
        # *** END YOUR CODE *** #
        self.arcs.reverse()
        # print(f"right most end, selfarcs: {self.arcs}\n")
        # print(f"right most end, deps: {deps}\n")
        return deps
    ...
    def get_oracle(self, graph: DependencyGraph):
        ...
        *ALSO IMPORTANT* make sure to use the values `self.left_arc_id`,
        ...
        if len(self.stack) == 2:
            if self.next == len(self.sentence):
                if self.stack[-1] >= graph.nodes[self.stack[0]]['address']:
                    for deprel, deps in graph.nodes[self.stack[0]]['deps'].items():
                        for dep in deps:
                            if dep == self.stack[-1]:
                                transition, deprel_label = self.right_arc_id, deprel       
                else:
                    raise ValueError
            else:
                transition, deprel_label = self.shift_id, None
            #print(f"tran, dep: {transition}, {deprel_label}\n")
            return transition, deprel_label
        if len(self.stack) > 2:
        # Access the indices of the top two items on the stack
            s1 = self.stack[-1]
            s2 = self.stack[-2]
            
            # Check if s2 is the head of s1 and that this arc has not been created yet
            # if get_head(s2, graph) == s1 and not any(arc for arc in self.arcs if arc[0] == s2 and arc[1] == s1):
            #     transition, deprel_label = self.left_arc_id, get_deprel(s2, graph)

            if s2 in get_left_deps(s1, graph) and not any(arc for arc in self.arcs if arc[0] == s2 and arc[1] == s1):
                transition, deprel_label = self.left_arc_id, get_deprel(s2, graph)
            
            # Check if s1 is the head of s2, the arc hasn't been created yet, and there are no remaining dependents of s1 in the buffer
            elif s1 in get_right_deps(s2, graph) and not any(arc for arc in self.arcs if arc[0] == s1 and arc[1] == s2) and all(dep < self.next for dep in get_right_deps(s1, graph)):
                transition, deprel_label = self.right_arc_id, get_deprel(s1, graph)

            # If neither left-arc nor right-arc conditions are satisfied, then shift
            else:
                transition, deprel_label = self.shift_id, None
    ...
    while unfinished_parses:
        ...
        for i, pp in enumerate(minibatch):
            ...
            try:
                ...
                # if parse is complete, mark for delete
                if pp.complete:
                    delete.append(i)
            except ValueError:
                delete.append(i)
