...
class ParserModel(nn.Module):
    ...
    def create_net_layers(self) -> None:
        ...
        input_size = N * self.config.embed_size
        self.hidden_layer = nn.Linear(input_size, self.config.hidden_size)
    ...
    def reshape_embedded(self, input_batch: torch.Tensor) -> torch.Tensor:
        ...
        """
        # *** BEGIN YOUR CODE *** #
        B = input_batch.size(0)  # Batch size
        reshaped_batch = input_batch.view(B, -1)
        # *** END YOUR CODE *** #
    ...
    def get_concat_embeddings(self, word_id_batch: torch.Tensor,
        ...
        """
        # *** BEGIN YOUR CODE *** #
        word_embedded = self.word_embed(word_id_batch)
        tag_embedded = self.tag_embed(tag_id_batch)
        deprel_embedded = self.deprel_embed(deprel_id_batch)

        word_reshaped = self.reshape_embedded(word_embedded)
        tag_reshaped = self.reshape_embedded(tag_embedded)
        deprel_reshaped = self.reshape_embedded(deprel_embedded)
        x = torch.cat((word_reshaped, tag_reshaped, deprel_reshaped), dim=1)
    ...
    def forward(self,
        ...
        h = torch.relu(h)
        h_drop = torch.dropout(h, p=self.config.dropout, train=self.training)
