...
class GraphDepModel(nn.Module):
    ...
    def create_arc_layers(self) -> None:
        ...
        * Initialize self.arc_W and self.arc_B according to a uniform
        ...
        self.arc_W = nn.Parameter(torch.Tensor(cfg.ARC_DIM, cfg.ARC_DIM))
        self.arc_B = nn.Parameter(torch.Tensor(cfg.ARC_DIM))
        nn.init.uniform_(self.arc_W, -sqrt(3.0 / cfg.ARC_DIM), sqrt(3.0 / cfg.ARC_DIM))
        nn.init.uniform_(self.arc_B, -sqrt(3.0 / cfg.ARC_DIM), sqrt(3.0 / cfg.ARC_DIM))
        # *** END YOUR CODE *** #
    ...
    def score_arcs(self, arc_head: Tensor, arc_dep: Tensor) -> Tensor:
        ...
        # *** BEGIN YOUR CODE *** #
        # not so sure
        part1 = torch.einsum('bxi,ij,byj->byx', arc_dep, self.arc_W, arc_head)
        part2 = torch.einsum('byi,i->by', arc_head, self.arc_B)

        arc_scores = part1 + part2.unsqueeze(-1)
        # *** END YOUR CODE *** #
        return arc_scores
    ...
    def create_label_layers(self) -> None:
        ...
        * Initialize self.label_B to zeros.
        ...
        self.label_h_mlp = nn.Sequential(
            nn.Linear(self.pt_width, cfg.LABEL_DIM),
            nn.ReLU(),
            nn.Dropout(cfg.DROPOUT),
        ...
        self.label_W = nn.Parameter(torch.Tensor(cfg.LABEL_DIM, cfg.LABEL_DIM, self._n_deprels))
        self.label_h_W = nn.Parameter(torch.Tensor(cfg.LABEL_DIM, self._n_deprels))
        self.label_d_W = nn.Parameter(torch.Tensor(cfg.LABEL_DIM, self._n_deprels))
        self.label_B = nn.Parameter(torch.zeros(cfg.LABEL_DIM))
        nn.init.uniform_(self.label_W, -sqrt(3.0 / cfg.LABEL_DIM), sqrt(3.0 / cfg.LABEL_DIM))
        nn.init.uniform_(self.label_h_W, -sqrt(3.0 / cfg.LABEL_DIM), sqrt(3.0 / cfg.LABEL_DIM))
        nn.init.uniform_(self.label_d_W, -sqrt(3.0 / cfg.LABEL_DIM), sqrt(3.0 / cfg.LABEL_DIM))
        # *** END YOUR CODE *** #
    ...
    def score_labels(self, label_head: Tensor, label_dep: Tensor) -> Tensor:
        ...
        # *** BEGIN YOUR CODE *** #
        #really not so sure :(
        label_scores = torch.einsum("byd,ddh,bxd -> byxd", label_dep, self.label_W, label_head)
        label_scores += torch.einsum("bid,dh->bid", label_head, self.label_h_W).unsqueeze(2)
        label_scores += torch.einsum("byd,dh->byd", label_dep, self.label_d_W).unsqueeze(2)
        label_scores += self.label_B.unsqueeze(0).unsqueeze(1).unsqueeze(2)
    ...
    def mask_possible(self, shape: T.Sequence[int]):
        ...
        # *** BEGIN YOUR CODE *** #
        # no word can point to itself - false to diagnal
        for i in range(shape[2]):
            mask[:, i, i, :] = False

        # only the root label can be assigned to the first word (0), others (1:) would be false
        mask[:, :, 0, 1:] = False
        
        # only point from the root, not to the root
        mask[:, 1:, 1:, 0] = False
            # *** END YOUR CODE *** #
        return mask
